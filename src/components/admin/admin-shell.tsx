...
export function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const expired = () => router.replace("/admin/login");
  ...
  return (
    <Access.Provider value={session ?? { id: "", email: "", permissions: [] }}>
      <div className="admin-app">
        ...
        <aside className={`admin-sidebar ${open ? "is-open" : ""}`}>
          ...
          <nav aria-label="Điều hướng quản trị">
            {navigation
              .filter(
                (n) => !session || session.permissions.includes(n.permission),
              )
              .map((n) => (
                <div key={n.href}>
                  {n.group && <p className="admin-nav-group">{n.group}</p>}
                  <Link
                    onClick={() => setOpen(false)}
                  ...
                  >
                    <n.icon size={19} />
                    <span>{n.label}</span>
                    {pathname === n.href && <ChevronRight size={15} />}
                  </Link>
                </div>
              ))}
          </nav>
          <div className="admin-sidebar-bottom">
            <div className="admin-help">
              <CircleHelp size={20} />
              <b>Một quy trình, trọn kỳ nghỉ.</b>
              <p>Theo dõi từ đặt phòng đến khi khách trả phòng.</p>
              <Link href="/admin/bookings">
                Quản lý booking <ArrowUpRight size={14} />
              </Link>
            </div>
            <Link href="/vi" className="admin-site-link">
        ...
        <div className="admin-main">
          <header className="admin-topbar">
            ...
            <div className="admin-user">
              ...
              <button onClick={logout} aria-label="Đăng xuất" title="Đăng xuất">
                <LogOut size={17} />
              </button>
            </div>
          </header>
          <main className="admin-content">
            {error ? (
              <div className="admin-error" role="alert">
                ...
                <Link href="/admin/login">Đổi tài khoản</Link>
              </div>
            ) : !session ? (
              <div className="admin-loading">Đang xác thực quyền truy cập…</div>
            ) : !allowed ? (
              <div className="admin-empty">
                <ShieldCheck />
                <h2>Bạn chưa có quyền truy cập mục này</h2>
                <p>
                  Chọn một mục trong menu hoặc liên hệ quản trị viên để được cấp
                  quyền.
                </p>
              </div>
            ) : (
