    ...
    <>
                ...
                <tbody>
                  {rows.map((c) => (
                    <tr key={c.id}>
                      ...
                      <td>{c._count?.bookings ?? 0} lượt đặt</td>
      ...
      {edit && (
        <FormModal
          ...
          saved={query.reload}
          fields={[
            { name: "lastName", label: "Họ", value: c?.lastName },
...
export function UnitManager({ pricing = false }: { pricing?: boolean }) {
  const query = useData<Unit[]>("units"),
    { permissions } = useAdmin();
  const [search, setSearch] = useState(""),
    [edit, setEdit] = useState<Unit>();
  const canEdit = ["unit.update", "pricing.update", "unit.publish"].every((p) =>
    permissions.includes(p),
  );
  const rows = (query.data ?? []).filter((u) =>
    `${u.nameVi} ${u.publicCode}`.toLowerCase().includes(search.toLowerCase()),
  );
  return (
    <>
      <Heading
        title={pricing ? "Giá phòng" : "Danh mục căn hộ"}
        description={
          pricing
            ? "Quản lý giá cơ bản mỗi đêm. Báo giá booking áp dụng thêm phụ phí và giá theo ngày đã cấu hình."
            : "Một góc nhìn đầy đủ về không gian lưu trú của bạn."
        }
      />
      <section className="admin-panel">
        <Toolbar search={search} setSearch={setSearch} />
        <DataState {...query}>
          {rows.length ? (
            <div className="admin-unit-grid">
              {rows.map((u) => (
                <article className="admin-unit-card" key={u.id}>
                  <div className="admin-unit-cover">
                    <Building2 size={44} strokeWidth={1} />
                    <span>{u.publicCode}</span>
                    <Badge value={u.status} />
                  </div>
                  <div className="admin-unit-info">
                    <small>{u.property?.name}</small>
                    <h2>{u.nameVi}</h2>
                    <p>
                      {u.bedroomCount} phòng ngủ <i /> {u.maxGuests} khách <i />{" "}
                      {Number(u.area)} m²
                    </p>
                    <div>
                      <strong>
                        {money(u.basePrice, u.currency)}
                        <small> / đêm</small>
                      </strong>
                      {canEdit && (
                        <button
                          className="admin-icon-button"
                          aria-label={`Sửa ${u.nameVi}`}
                          onClick={() => setEdit(u)}
                        >
                          <Pencil size={17} />
                        </button>
      ...
      {edit && (
        <FormModal
          title="Cập nhật căn hộ & giá"
          path={`units/${edit.id}`}
          method="PATCH"
          close={() => setEdit(undefined)}
          saved={query.reload}
          fields={[
            { name: "nameVi", label: "Tên căn hộ", value: edit.nameVi },
            {
              name: "basePrice",
              label: `Giá cơ bản / đêm (${edit.currency})`,
              type: "number",
              min: 0,
              value: Number(edit.basePrice),
            },
            {
              name: "maxGuests",
