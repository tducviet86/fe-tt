    ...
    <group ref={group} rotation={[0.18, -0.5, -0.08]}>
      <Float speed={1.6} rotationIntensity={0.12} floatIntensity={0.35}>
        <RoundedBox args={[2.9, 1.65, 0.22]} radius={0.18} smoothness={5}>
          <MeshDistortMaterial color="#d8794b" roughness={0.35} metalness={0.08} distort={0.12} speed={1.4} />
        </RoundedBox>
        <RoundedBox args={[1.75, 1.08, 0.3]} radius={0.14} smoothness={5} position={[-0.1, 0.02, 0.22]}>
      ...
      <Sparkles count={18} scale={[4, 2.6, 2]} size={2.2} speed={0.25} color="#f7dec0" />
    </group>
  );
}
...
export function SearchAtmosphere() {
  ...
  return (
    <div className="absolute inset-0" aria-hidden="true">
      <Canvas camera={{ position: [0, 0, 5.2], fov: 38 }} dpr={[1, 1.5]} frameloop={reduceMotion ? "demand" : "always"} gl={{ alpha: true, antialias: true }}>
        <ambientLight intensity={1.8} />
        <directionalLight position={[3, 4, 5]} intensity={3.5} color="#fff1dd" />
